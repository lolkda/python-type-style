...
- **`SKILL.md`** holds "Quick Rules" sections (Typing, Response, FastAPI, SQLAlchemy 2, Pydantic v2, Property, Async) plus an Anti-patterns list and a References Index table mapping each topic to its reference file and example file. Treat it as the index; don't inline deep treatments here.
...
- **Bilingual on purpose.** Prose and code comments are English; `Field(description=...)`, route `summary=...`, and Chinese docstring sections (用途/Args/Returns) use Chinese (中文). This is one of the rules the skill enforces — do not "normalize" example code to all-English descriptions.
