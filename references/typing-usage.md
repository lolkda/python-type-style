...
| `TypedDict` | Use only for raw external `dict` ingress immediately before `BaseModel.model_validate(...)`. | `Unpack[TypedDict]` is allowed only inside a third-party, framework, or external I/O adapter for typed kwargs passed directly to that boundary call. | `TypedDict` as request/config/domain state, outward API contract, or project-function contract; use `BaseModel`. |
